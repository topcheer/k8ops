...
| 归属问责引擎 | `/api/product/ownership-map` | Product | 工作负载归属映射，孤儿资源检测，团队问责评分，标签覆盖率分析 |
| 平台成熟度引擎 | `/api/docs/platform-maturity` | Documentation | CMMI成熟度评估，六大维度评分，盲区覆盖分析，演进路线图 |
| 合规态势引擎 | `/api/security/compliance-posture` | Security | SOC2/PCI-DSS/HIPAA/NIST/GDPR 多框架映射，跨框架差距分析，修复优先级 |
| 可观测覆盖引擎 | `/api/operations/obs-coverage` | Operations | 监控盲区检测，信号覆盖率（metrics/tracing/dashboard/runbook/alerts），盲点指数 |
| 配置一致性引擎 | `/api/deployment/config-consistency` | Deployment | 配置偏差检测，镜像源分析，资源分层，标准化评分 |
| 调度智能引擎 | `/api/scalability/scheduling-intel` | Scalability | 装箱效率评分，节点碎片化分析，调度适配评估，闲置资源量化 |
| 依赖韧性引擎 | `/api/product/dependency-resilience` | Product | 服务依赖链分析，孤儿服务检测，单Pod风险，级联故障评估 |
| 变更智能引擎 | `/api/operations/change-intel` | Operations | 变更爆炸半径分析，变更速率追踪，风险变更关联，变更冻结建议 |
| 网络隔离引擎 | `/api/security/net-policy-effectiveness` | Security | 网络策略有效性，零信任评分，命名空间隔离，默认拒绝检测 |
| 资源治理引擎 | `/api/deployment/resource-governance` | Deployment | 命名空间配额覆盖，LimitRange默认值，配额使用率，治理评分 |
| 网格就绪引擎 | `/api/product/mesh-readiness` | Product | 服务网格检测，mTLS覆盖率，sidecar注入差距，韧性策略缺失 |
| 闲置浪费引擎 | `/api/scalability/idle-waste` | Scalability | 零副本工作负载，未挂载PVC，LB成本，月度浪费估算 |
| 策略治理引擎 | `/api/security/policy-governance` | Security | OPA/Gatekeeper检测，PSA覆盖率，策略执行差距，准入审计 |
| API质量引擎 | `/api/docs/api-quality` | Documentation | 维度覆盖率，端点统计，差距检测，质量评分 |
| 数据基数引擎 | `/api/operations/obs-cardinality` | Operations | 指标基数爆炸，日志量分布，采集器健康，数据成本估算 |
| GitOps漂移引擎 | `/api/deployment/gitops-drift` | Deployment | ArgoCD/Flux检测，手动部署识别，ConfigMap过期，漂移评分 |
| API版本治理引擎 | `/api/product/api-version-governance` | Product | 弃用API检测，alpha/beta使用，CRD版本健康，升级就绪 |
| Secret生命周期引擎 | `/api/security/secret-lifecycle` | Security | Secret年龄，轮换合规，明文检测，散布分析，未使用清理 |
| DR备份引擎 | `/api/scalability/dr-backup-verify` | Scalability | 备份工具检测，命名空间覆盖，PVC保护，RPO/RTO估算 |
| 上岗培训引擎 | `/api/docs/training-readiness` | Documentation | Owner/Team/Docs标签覆盖，文档完整性，知识传递就绪 |
| 证书过期引擎 | `/api/operations/cert-expiry` | Operations | TLS证书解析，过期检测，自签名识别，续期提醒 |
| 供应链安全引擎 | `/api/security/image-supply-chain` | Security | 注册表信任，摘要固定，:latest检测，拉取策略审计 |
| 节点OS漂移引擎 | `/api/scalability/node-os-drift` | Scalability | 内核版本漂移，运行时一致性，节点年龄，GPU可用性 |
| 流量拓扑引擎 | `/api/product/traffic-flow` | Product | 服务暴露级别，端点健康，孤儿服务检测，LB审计 |
