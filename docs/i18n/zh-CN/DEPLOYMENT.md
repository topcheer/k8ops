...
# 访问 Dashboard
# 浏览器打开 https://<您的域名>  (如 https://k8ops.iot2.win)
...
| 探针 | 路径 | 作用 | 参数 |
|------|------|------|------|
| **startupProbe** | `/healthz` | 等待启动完成（防止慢启动被 liveness 杀死） | failureThreshold: 30, period: 5s（最长等待 150s） |
| **livenessProbe** | `/healthz` | 存活检查（失败则重启 Pod） | period: 20s, failureThreshold: 3, timeout: 5s |
| **readinessProbe** | `/readyz` | 就绪检查（失败则从 Service Endpoints 移除） | period: 10s, failureThreshold: 3, timeout: 5s |
...
# --pre-stop 会 sleep 5s，等待 Ingress Controller 从负载均衡中移除该 Pod
# 然后 kubelet 发送 SIGTERM，触发 dashboard 优雅关闭（排空 SSE 连接）
# terminationGracePeriodSeconds: 30 确保有足够时间完成
```

停机流程：
1. kubelet 执行 `preStop` → sleep 5s（连接排空）
2. kubelet 发送 SIGTERM → Go 信号处理器开始优雅关闭
3. Dashboard HTTP 服务器停止接受新请求
4. SSE 连接排空（10s 超时）
5. Controller Manager 优雅关闭
6. 进程退出
