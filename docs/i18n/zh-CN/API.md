...
所有端点均在 dashboard 端口上提供服务（默认 `:9090`）。

**认证：** JWT cookie（`k8ops_token`）或 `Authorization: Bearer <token>` 头。
...
| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/health` | 无 | 存活探针 — 返回 `{"status":"ok"}` |
| GET | `/api/version` | 无 | 构建版本、git commit、Go 版本 |

## 集群

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/cluster/overview` | 需要 | 集群摘要：节点数、Pod 数、CPU/内存使用率、告警（30s 缓存） |
| GET | `/api/nodes` | 需要 | 列出所有节点及其资源使用和条件状态（30s 缓存） |
| GET | `/api/nodes/{node}/pods` | 需要 | 特定节点上运行的 Pod |
| GET | `/api/pods` | 需要 | 列出所有命名空间的 Pod（30s 缓存） |
| GET | `/api/pods/{namespace}/{name}/containers` | 需要 | Pod 的容器列表 |
| GET | `/api/pods/{namespace}/{name}/logs?container=&follow=&tailLines=` | 需要 | Pod 日志（`follow=true` 时支持 SSE 流式传输） |
| GET | `/api/events?namespace=&warning=` | 需要 | Kubernetes 事件，可按命名空间/告警过滤 |
| GET | `/api/resources?kind=&namespace=` | 需要 | 通用资源列表器（Deployment、Service 等）（60s 缓存） |
| GET | `/api/crds?with_counts=true` | 需要 | 自定义资源定义（带计数 10min 缓存） |
| GET | `/api/crd-resources?group=&version=&resource=&namespace=` | 需要 | CRD 实例（60s 缓存） |
| GET | `/api/yaml?namespace=&name=&group=&version=&resource=&kind=` | 需要 | 任意 Kubernetes 资源的 YAML 视图 |

## 诊断与修复

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/diagnostics` | 需要 | 列出 DiagnosticReport CR，可选 `?namespace=` 过滤 |
| GET | `/api/diagnostics/{namespace}/{name}` | 需要 | 诊断详情（含 AI 分析） |
| GET | `/api/remediations` | 需要 | 列出 Remediation CR，可选 `?namespace=` 过滤 |
| GET | `/api/optimizations` | 需要 | 列出 Optimization CR，可选 `?namespace=` 过滤 |

## AI 对话

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| POST | `/api/chat` | 需要 | 向 AI 助手发送消息（SSE 流式响应） |
| GET | `/api/chat/conversations?id=` | 需要 | 列出会话或按 ID 获取单个会话 |
...
**响应：** SSE 流式返回 AI 分析及工具调用结果。

### GET /api/chat/conversations

返回会话历史。传入 `?id=<uuid>` 获取单个会话。

## Provider 管理

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/provider/status` | 需要 | 当前 AI Provider 配置（API 密钥已脱敏） |
| POST | `/api/provider/update` | 需要 | 运行时更新 Provider 类型/模型/端点 |
| POST | `/api/provider/reload` | 需要 | 从 K8opsConfig CRD 重新加载 Provider 配置 |
| GET | `/api/tools` | 需要 | 列出已注册的诊断工具 |

## 认证

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| POST | `/api/auth/login` | 公开 | 本地登录（有速率限制） |
| POST | `/api/auth/logout` | 需要 | 清除认证 cookie |
| GET | `/api/auth/me` | 需要 | 当前用户信息 |
| POST | `/api/auth/change-password` | 需要 | 修改自己的密码 |
| GET | `/api/auth/status` | 公开 | 认证配置状态（auth_enabled、user_count、ldap/oidc 标志） |
| GET | `/api/auth/provider-presets` | 公开 | 可用的 OIDC/LDAP Provider 模板 |
...
设置 `k8ops_token` cookie（HttpOnly、SameSite=Lax、24h）。
...
| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/auth/oidc/{provider}/login` | 公开 | 重定向到 OIDC Provider（设置 CSRF state cookie） |
| GET | `/api/auth/oidc/{provider}/callback` | 公开 | OIDC 回调（验证 state，创建用户会话） |

## Auth Provider 管理（管理员）

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/auth/providers` | 管理员 | 列出已配置的认证 Provider |
| POST | `/api/auth/providers` | 管理员 | 创建认证 Provider（LDAP/OIDC） |
| GET | `/api/auth/providers/{id}` | 管理员 | 按 ID 获取 Provider |
| PUT | `/api/auth/providers/{id}` | 管理员 | 更新 Provider 配置 |
| DELETE | `/api/auth/providers/{id}` | 管理员 | 删除 Provider |
...
| POST | `/api/admin/users` | 管理员 | 创建用户（默认角色：viewer，MustChangePwd=true） |
| GET | `/api/admin/users/{id}` | 管理员 | 按 ID 获取用户 |
| PUT | `/api/admin/users/{id}` | 管理员 | 更新用户（角色、命名空间等） |
| DELETE | `/api/admin/users/{id}` | 管理员 | 删除用户 |
| POST | `/api/admin/users/{id}/reset-password` | 管理员 | 重置密码（设置 MustChangePwd=true） |
| GET | `/api/admin/auth-config` | 管理员 | 获取认证配置 |
| PUT | `/api/admin/auth-config` | 管理员 | 更新认证配置 |

## API 密钥

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/auth/api-keys` | 需要 | 列出自己的 API 密钥 |
| POST | `/api/auth/api-keys` | 需要 | 创建 API 密钥 |
| DELETE | `/api/auth/api-keys/{id}` | 需要 | 撤销 API 密钥 |

## RBAC 管理（管理员）

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/rbac/clusterroles` | 管理员 | 列出 ClusterRole |
| GET | `/api/rbac/clusterroles/{name}` | 管理员 | 按名称获取 ClusterRole |
| DELETE | `/api/rbac/clusterroles/{name}` | 管理员 | 删除 ClusterRole |
| GET | `/api/rbac/roles?namespace=` | 管理员 | 列出命名空间 Role |
| GET | `/api/rbac/roles/{namespace}/{name}` | 管理员 | 获取命名空间 Role |
| DELETE | `/api/rbac/roles/{namespace}/{name}` | 管理员 | 删除命名空间 Role |
| GET | `/api/rbac/rolebindings?namespace=` | 管理员 | 列出 RoleBinding |
| GET | `/api/rbac/rolebindings/{namespace}/{name}` | 管理员 | 获取 RoleBinding |
| DELETE | `/api/rbac/rolebindings/{namespace}/{name}` | 管理员 | 删除 RoleBinding |
| GET | `/api/rbac/api-resources` | 管理员 | 列出 Kubernetes API 资源类型 |
| GET | `/api/rbac/namespaces` | 管理员 | 列出所有命名空间 |
| GET | `/api/rbac/role-mapping?role=&kind=&name=&namespace=` | 管理员 | 查看角色到主体的映射 |
| GET | `/api/rbac/role-defs` | 管理员 | 列出 k8ops 自定义角色定义 |
| GET | `/api/rbac/subjects?kind=&namespace=` | 管理员 | 列出主体（用户/组/ServiceAccount） |

## 审计

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/audit?namespace=&limit=` | 需要 | 审计日志条目（分页） |
| GET | `/api/audit/stats` | 需要 | 审计统计摘要 |

## 配置

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/config` | 需要 | k8ops 控制器配置（Provider 类型/模型、功能特性） |

## 安全审计

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | `/api/security/audit` | 需要 | 集群安全扫描 — 检查 Pod Security Standards、RBAC、网络策略覆盖率、密钥安全 |
| GET | `/api/security/health` | 需要 | 平台安全健康检查 — 认证/TLS/K8s API 连接性 |
...
**检查项：**
- **Pod 安全：** 特权容器、root 运行、权限提升、危险 capabilities、hostPath/hostNetwork
- **RBAC：** cluster-admin 绑定、默认 SA 使用
- **网络：** 缺少 NetworkPolicy 的命名空间
- **密钥：** Docker registry 密钥轮换建议
- **资源：** 缺少 resource limits 的容器
...
| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| POST | `/api/scale` | 需要 | 扩缩容 deployment/statefulset |
| POST | `/api/pod/delete` | 需要 | 删除单个 Pod |
| POST | `/api/rollout/restart` | 需要 | 滚动重启 deployment/daemonset/statefulset |
| POST | `/api/node/cordon` | 需要 | 隔离/恢复节点 |
| POST | `/api/yaml/apply` | 需要 | 应用 YAML (kubectl apply) |
...
|--------|------|
| 400 | 请求错误（缺失/无效参数） |
| 401 | 未授权（缺失/过期/无效 token） |
| 403 | 禁止访问（角色权限不足） |
| 404 | 资源未找到 |
| 500 | 内部服务器错误 |
| 503 | 服务不可用（AI Provider 未配置） |

## 角色

| 角色 | 权限 |
|------|------|
| `admin` | 完全访问，包括用户/RBAC/Provider 管理 |
| `operator` | Dashboard + 诊断 + 对话（无用户管理） |
| `viewer` | 只读 Dashboard + 对话 |
| `ns-admin` | 仅在分配的命名空间内拥有管理员权限 |
| `ns-viewer` | 仅在分配的命名空间内拥有查看权限 |
...
| `effective_capacity` | 理论容量 vs 有效容量（不可调度节点导致的容量损失百分比） |
...
| 内存请求过高 (>2GB/container) | 建议合理调整资源大小 |
