...
### 95. GET /api/deployment/config-sync — ConfigMap/Secret Config Sync & Staleness Detector

Detects pods running with stale configuration after ConfigMap/Secret updates. Identifies env var refs (env/envFrom) that do NOT auto-update, subPath volume mounts that don't auto-update, and workloads missing Reloader annotations. Cross-references pod start time with ConfigMap/Secret timestamps. Immutable ConfigMap/Secret detection. Staleness score (0-100).

---

### 96. GET /api/operations/kubelet-health — Kubelet & Container Runtime Health Monitor

Monitors kubelet and container runtime health across all nodes. Per-node: kubelet version, runtime version, OS image, heartbeat age, active conditions. Version skew and runtime skew detection. Runtime type distribution (containerd/docker/cri-o). Health score (0-100).
...
### 107. GET /api/scalability/node-topology — Node Topology Distribution & Multi-AZ Fault Tolerance Analyzer

Analyzes node distribution across availability zones and regions. Per-zone: node count, CPU/memory allocation, pod count. Single-zone cluster detection. Zone imbalance detection. Missing zone label detection. Multi-AZ fault tolerance assessment. Health score (0-100).

---

### 108. GET /api/security/rbac-audit — RBAC Overprivilege & Wildcard Permission Auditor

Audits RBAC roles for overprivilege. Detects wildcard verbs (*), wildcard resources (*), non-system cluster-admin bindings, and excessive RoleBindings. Per-role severity classification. System role filtering. Health score (0-100).

---

### 109. GET /api/product/backup-compliance — Volume Snapshot & PVC Backup Compliance Auditor

Audits PVC backup and snapshot compliance. Detects unprotected PVCs in use, critical large PVCs (>=1Gi) without backup, Velero installation status. Per-namespace and per-storage-class compliance. Health score (0-100).
...
### v16.97: API Documentation Sync v16.89-v16.96
- CHANGELOG + API.md + en/API.md synced for v16.89-v16.96

### v16.98: Observability Stack Integration Health Auditor
- `GET /api/operations/observability-stack` — Three pillars (metrics, logging, tracing) integration health: backend detection (Prometheus, Loki, Jaeger, Tempo, OpenTelemetry), per-pillar agent DaemonSet coverage, backend pod readiness, namespace coverage. Health score (0-100), 4 unit tests.

### v16.99: Secret Management Posture & External Secret Integration Auditor
- `GET /api/security/secret-posture` — External Secrets Operator, Sealed Secrets, SOPS, HashiCorp Vault detection. Managed/unmanaged/plaintext classification, empty/large secrets, namespace risk levels. Integration status (integrated/partial/missing). Health score (0-100), 4 unit tests.

### v17.00: Node Drain & Rotation Readiness Auditor
- `GET /api/scalability/node-drain-readiness` — Per-node drain safety: StatefulSet sticky pods (PVC binding), bare pods (will be lost), local storage (data loss), PDB-protected pods, DaemonSet pods, cordoned nodes, single-replica workloads. Per-node classification (safe/risky/dangerous/cordoned). Health score (0-100), 4 unit tests.

### v17.01: East-West Traffic & Service-to-Service Connectivity Auditor
...
### v17.09: API Documentation Sync v17.03-v17.08
- CHANGELOG + API.md + en/API.md synced for v17.03-v17.08

### v17.10: Pod Restart Pattern & CrashLoop Clustering Auditor
- `GET /api/operations/restart-storm` — High restart detection (>5/>20), namespace clustering, same-image cascade, hotspot pods. 4 unit tests.

### v17.11: Container Image Provenance & Registry Trust Auditor
- `GET /api/security/image-provenance` — Trusted vs untrusted registries, digest pinning, :latest tag detection, per-registry/namespace stats. 4 unit tests.

### v17.12: Pod Resource Request Density & Scheduling Fit Auditor
- `GET /api/scalability/scheduling-fit` — Per-node packing %, over/under-provisioned pods, no-request pods, bin-packing efficiency, fit category. 4 unit tests.

### v17.13: Service Endpoint vs Pod Readiness Mismatch Auditor
- `GET /api/product/endpoint-mismatch` — Dead services, stale endpoints, selector matching, per-namespace stats. 4 unit tests.
