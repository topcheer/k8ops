...
func TestStorageOrphanTypes(t *testing.T) {
	...
	if o.SizeGB != 10.0 {
		t.Error("orphan error")
	}
}

func TestStorageOrphanScoring(t *testing.T) {
	tests := []struct {
		total, orphan, pending int
		expMin, expMax         int
	}{
		{15, 0, 0, 95, 100},
		{15, 5, 2, 50, 80},
	...
	for _, tc := range tests {
		...
		orphanRatio := 0.0
		if tc.total > 0 {
			orphanRatio = float64(tc.orphan) / float64(tc.total)
		}
