...
func TestCapacityForecastTypes(t *testing.T) {
	...
	if r.HealthScore != 70 { t.Error("struct error") }
	s := CapForecastSummary{NodeCount: 3, CPUPct: 65.5, MemPct: 40.0}
	if s.CPUPct != 65.5 { t.Error("summary error") }
	f := CapForecast{Resource: "CPU", UsagePct: 90, Severity: "high"}
	if f.Severity != "high" { t.Error("forecast error") }
}

func TestCapacityForecastScoring(t *testing.T) {
	tests := []struct{ cpuPct, memPct, podPct float64; expMin, expMax int }{
		{30, 30, 20, 90, 100},
		{95, 90, 85, 35, 55},
		{60, 50, 40, 90, 100},
	}
