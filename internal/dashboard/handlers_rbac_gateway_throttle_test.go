...
func TestGatewayHealthTypes(t *testing.T) {
	...
	if !s.ControllerRunning || s.TotalIngresses != 10 {
		t.Error("summary error")
	}
	ce := GatewayControllerEntry{Name: "traefik", Ready: true, Type: "Traefik"}
	if !ce.Ready {
		t.Error("controller error")
	}
...
func TestThrottleRiskScoring(t *testing.T) {
	...
	score := 100
	if score != 100 {
		t.Errorf("clean should be 100, got %d", score)
	}
