...
func TestProbeLatencyScoring(t *testing.T) {
	...
	for _, tc := range tests {
		score := 0
		if tc.total > 0 {
			score = tc.ready * 60 / tc.total
		}
		if tc.total > 0 {
			score += tc.startup * 20 / tc.total
		}
		score -= tc.slow * 5
		score -= tc.missing * 3
		if score < 0 {
...
func TestHelmHealthTypes(t *testing.T) {
	...
	if s.FailedReleases != 0 || s.StaleReleases != 5 {
		t.Error("summary error")
	}
...
func TestSpotReadinessTypes(t *testing.T) {
	...
	if sw.HasSpotToleration {
		t.Error("spotWL error")
	}
